...
| Defaults: `{}` config/env, `[]` args; Codex model/effort | `agent.go` create path | `RuntimeConfig`→`{}`, `CustomEnv`→`{}`, `CustomArgs`→`[]` when nil; Codex empty model/effort → `gpt-5.6-sol`/`medium` before insert |
